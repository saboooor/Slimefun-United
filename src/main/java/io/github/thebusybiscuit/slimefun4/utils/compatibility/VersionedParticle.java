...
public class VersionedParticle {
    ...
    static {
        ...
        SMOKE = version.isAtLeast(1, 20, 5) ? Particle.SMOKE : getKey("SMOKE_NORMAL");

        // VILLAGER_HAPPY is renamed to HAPPY_VILLAGER in 1.20.5
        HAPPY_VILLAGER = version.isAtLeast(1, 20, 5) ? Particle.HAPPY_VILLAGER : getKey("VILLAGER_HAPPY");

        // CRIT_MAGIC is renamed to ENCHANTED_HIT in 1.20.5
        ENCHANTED_HIT = version.isAtLeast(1, 20, 5) ? Particle.ENCHANTED_HIT : getKey("CRIT_MAGIC");

        // EXPLOSION_LARGE is renamed to EXPLOSION in 1.20.5
        EXPLOSION = version.isAtLeast(1, 20, 5) ? Particle.EXPLOSION : getKey("EXPLOSION_LARGE");

        // SPELL_WITCH is renamed to WITCH in 1.20.5
        WITCH = version.isAtLeast(1, 20, 5) ? Particle.WITCH : getKey("SPELL_WITCH");

        // FIREWORKS_SPARK is renamed to FIREWORK in 1.20.5
        FIREWORK = version.isAtLeast(1, 20, 5) ? Particle.FIREWORK : getKey("FIREWORKS_SPARK");

        // ENCHANTMENT_TABLE is renamed to ENCHANT in 1.20.5
        ENCHANT = version.isAtLeast(1, 20, 5) ? Particle.ENCHANT : getKey("ENCHANTMENT_TABLE");
    }
