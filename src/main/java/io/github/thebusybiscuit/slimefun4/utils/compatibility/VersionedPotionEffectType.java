...
public class VersionedPotionEffectType {
    ...
    static {
        ...
        HASTE = version.isAtLeast(1, 20, 5) ? PotionEffectType.HASTE : getKey("FAST_DIGGING");

        MINING_FATIGUE = version.isAtLeast(1, 20, 5) ? PotionEffectType.MINING_FATIGUE : getKey("SLOW_DIGGING");

        STRENGTH = version.isAtLeast(1, 20, 5) ? PotionEffectType.STRENGTH : getKey("INCREASE_DAMAGE");

        INSTANT_HEALTH = version.isAtLeast(1, 20, 5) ? PotionEffectType.INSTANT_HEALTH : getKey("HEAL");

        INSTANT_DAMAGE = version.isAtLeast(1, 20, 5) ? PotionEffectType.INSTANT_DAMAGE : getKey("HARM");

        JUMP_BOOST = version.isAtLeast(1, 20, 5) ? PotionEffectType.JUMP_BOOST : getKey("JUMP");

        NAUSEA = version.isAtLeast(1, 20, 5) ? PotionEffectType.NAUSEA : getKey("CONFUSION");

        RESISTANCE = version.isAtLeast(1, 20, 5) ? PotionEffectType.RESISTANCE : getKey("DAMAGE_RESISTANCE");
    }
